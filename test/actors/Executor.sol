...
contract Executor is Test {
        ...
        if (
            (
                block.timestamp >= epochEndTime - coordinator.getEpochDuration() + coordinator.getCommitPhaseDuration()
                    && block.timestamp < epochEndTime
            ) || block.timestamp < minimumUnstakeTime
        ) {
    ...
    function executeBatch() public {
        ...
        if (
            block.timestamp < coordinator.epochEndTime() - coordinator.getSlashingDuration()
                && block.timestamp
                    >= coordinator.epochEndTime() - coordinator.getEpochDuration() + coordinator.getSelectionPhaseDuration()
        ) {
            // we are in rounds
            uint256 timeIntoRounds = coordinator.getEpochDuration() - coordinator.getSelectionPhaseDuration()
                - (coordinator.epochEndTime() - block.timestamp);
            ...
            if (inRound) {
                ...
                while (true) {
                    uint256 executorIndex =
                        uint256(keccak256(abi.encodePacked(seed, round))) % coordinator.numberOfActiveExecutors();
    ...
    function slashCommitter(address _executor) public {
        ...
        coordinator.setCommitment(
            ICoordinator.CommitData({commitment: 0, epoch: coordinator.epoch(), revealed: false}), _executor
        );
