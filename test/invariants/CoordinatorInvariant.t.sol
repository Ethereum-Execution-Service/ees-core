...
contract CoordinatorInvariant is Test {
    ...
    function setUp() public {
        ...
        targetSelector(FuzzSelector({addr: address(coordinatorHandler), selectors: selectors}));
    }
    ...
    function invariant_coordinatorTokenBalance() public view {
        ...
        assertEq(
            totalExecutorBalances + protocolBalance + nextEpochPoolBalance, coordinatorHandler.getCoordinatorBalance()
        );
    }
    ...
    function invariant_tokensInSystemEqualTotalStakedMinusTotalUnstaked() public view {
        ...
        assertEq(
            ...
            coordinatorHandler.getTotalExecutorBalances() + coordinatorHandler.getProtocolBalance()
                + coordinatorHandler.getTreasuryBalance() + coordinatorHandler.getNextEpochPoolBalance()
        );
    }
    ...
    function invariant_numberOfInitializedExecutorsGeActive() public view {
        (uint256 initializedExecutors, uint256 activeExecutors) =
            coordinatorHandler.getNumberOfInitializedAndActiveExecutors();
